...
class EagleDashboardDragonBoard(models.Model):
    _name = 'eagle_dashboard.board'
    _description = 'Dashboard Dragon'
    ...
    def create(self, vals):
        record = super(EagleDashboardDragonBoard, self).create(vals)
    ...
    @api.multi
    def write(self, vals):
        record = super(EagleDashboardDragonBoard, self).write(vals)
        for rec in self:
    ...
    def unlink(self):
        ...
        res = super(EagleDashboardDragonBoard, self).unlink()
        return res
    ...
    def eagle_import_dashboard(self, file):
        ...
        if 'eagle_file_format' in eagle_dashboard_file_read and eagle_dashboard_file_read[
            ...
            raise ValidationError(_("Current Json File is not properly formatted according to Dashboard Dragon Model."))
        ...
        for data in eagle_dashboard_data:
            if not all(key in data for key in eagle_dashboard_key):
                raise ValidationError(
                    _("Current Json File is not properly formatted according to Dashboard Dragon Model."))
            vals = {
                'name': data['name'],
...
class DashboardDragonTemplate(models.Model):
    _name = 'eagle_dashboard.board_template'
    _description = 'Dashboard Dragon Template'
    name = fields.Char()
